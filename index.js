class Todo {
  ...
  filter() {
    ...
    this.state.filteredItems = this.state.items.filter(({title}) => {
      ...
      return titleFormated.includes(queryFormated)
    })
    this.render()
  }
  ...
  // Обязательно делаем запись в стрелочной форме, чтобы this не потерял контекст
  ...
  onDeleteAllButtonClick = () => {
    const isConfirmed = confirm('Вы хотите удалить все?')
